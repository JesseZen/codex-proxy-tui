import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@agent-inn/plugin/tui"
...
import type { useAinnKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useAinnKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getAinnModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getAinnModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
